...
import interview.modules.knowledgebase.model.RagChatSessionEntity.SessionStatus;
...
public interface RagChatSessionRepository extends JpaRepository<RagChatSessionEntity, Long> {
    ...
    List<RagChatSessionEntity> findAllOrderByPinnedAndUpdatedAtDesc(@Param("userId") Long userId);
    ...
    @Query("SELECT DISTINCT s FROM RagChatSessionEntity s LEFT JOIN FETCH s.knowledgeBases WHERE s.id = :id AND s.userId = :userId")
    Optional<RagChatSessionEntity> findByIdAndUserIdWithMessagesAndKnowledgeBases(@Param("id") Long id, @Param("userId") Long userId);
